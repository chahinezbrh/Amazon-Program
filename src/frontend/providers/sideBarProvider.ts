...
export class SideBarProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(
    ...
    webviewView.webview.options = {
      enableScripts: true,
      localResourceRoots: [
        vscode.Uri.joinPath(this._extensionUri, 'out', 'webviews', 'sideBar'),
      ],
    };
      ...
      (message) => {
        switch (message.command) {
          case 'selectFunction':
            ...
            break;
          case 'recordMemory':
            vscode.commands.executeCommand('docManager.recordMemory');
            break;
          case 'openNotifications':
            vscode.commands.executeCommand('docManager.showNotificationCenter', message.filter);
            break;
        }
      },
      null,
  ...
  private _parseFunctions(document: vscode.TextDocument): FunctionItem[] {
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        // A capture group can be undefined if it didn't participate in the
        // match, so the compiler won't accept match[1] as a string.
        const name = match[1];
  ...
  private _getHtml(webview: vscode.Webview): string {
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'out', 'webviews', 'sideBar', 'sideBar.js')
    );
    const cssUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'out', 'webviews', 'sideBar', 'sideBar.css')
    );
