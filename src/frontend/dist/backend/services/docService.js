...
exports.toDocEntries = toDocEntries;
exports.getDocsForSymbol = getDocsForSymbol;
exports.saveDoc = saveDoc;
const docFileStore_1 = require("./docFileStore");
const toLines = (s) => s.split('\n');
const fromLines = (l) => (l ?? []).join('\n');
// ---------------------------------------------------------------------------
// Read
// ---------------------------------------------------------------------------
...
function toDocEntries(fn, filePath) {
    const entries = [];
    const loc = {
        symbolName: fn.name,
        filePath,
    ...
    if (fn.aiDocumentation?.length) {
        entries.push({
            ...
            isStale: fn.aiGeneratedAtHash !== undefined && fn.aiGeneratedAtHash !== fn.hash,
        });
    }
...
async function getDocsForSymbol(repoRoot, meta) {
    const functions = await (0, docFileStore_1.readFileDocs)(repoRoot, meta.filePath);
...
async function saveDoc(repoRoot, input) {
    ...
    if (fn.hash !== codeHash) {
        ...
        fn.hash = codeHash;
        fn.memories = fn.memories.map((m) => m.hashAtRecording !== codeHash ? { ...m, isStale: true } : m);
    }
    ...
    switch (input.type) {
        ...
        case 'voice': {
            const memory = {
                id: (0, docFileStore_1.newMemoryId)(),
                author: input.author,
    ...
    await (0, docFileStore_1.writeFileDocs)(repoRoot, meta.filePath, functions);
    const saved = toDocEntries(fn, meta.filePath).find((e) => e.id === savedId);
    if (!saved)
        throw new Error(`saveDoc: no ${input.type} entry after write`);
    return saved;
}
