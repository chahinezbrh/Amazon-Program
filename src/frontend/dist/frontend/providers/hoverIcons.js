...
exports.getAddMemorySvg = getAddMemorySvg;
exports.getAiDocsSvg = getAiDocsSvg;
exports.getWriteDocsSvg = getWriteDocsSvg;
exports.getFullDocsSvg = getFullDocsSvg;
exports.getNoMemorySvg = getNoMemorySvg;
exports.getVoiceMemoryLabelSvg = getVoiceMemoryLabelSvg;
exports.getVoiceMemorySvg = getVoiceMemorySvg;
exports.getFooterPlayMemorySvg = getFooterPlayMemorySvg;
exports.getFooterFullDocsSvg = getFooterFullDocsSvg;
exports.getPlayMemoryHeaderSvg = getPlayMemoryHeaderSvg;
...
function getAddMemorySvg() {
    return outlinedPill({ label: '+ Add memory', color: exports.COLORS.teal, width: 114, textX: 57 });
}
function getAiDocsSvg() {
    const sparkles = `<path d="M19 7.5L20.2 11.2L24 12.5L20.2 13.8L19 17.5L17.8 13.8L14 12.5L17.8 11.2Z" fill="${exports.COLORS.blue}"/>
    <path d="M24.5 6L25.2 7.8L27 8.5L25.2 9.2L24.5 11L23.8 9.2L22 8.5L23.8 7.8Z" fill="${exports.COLORS.blue}"/>`;
    return outlinedPill({ label: 'AI docs', color: exports.COLORS.blue, width: 98, leadingIcon: sparkles, textX: 60 });
}
function getWriteDocsSvg() {
    return outlinedPill({ label: 'Write docs', color: exports.COLORS.orange, width: 102, textX: 51 });
}
/** NEW — matches the same outlined-pill family as the other header buttons */
function getFullDocsSvg() {
    return outlinedPill({ label: 'Full docs', color: exports.COLORS.muted, width: 96, textX: 48 });
}
/** Row of text shown when the symbol has no recorded voice memory */
function getNoMemorySvg() {
    ...
    <text x="2" y="17" fill="${exports.COLORS.orange}" font-family="${FONT}" font-size="13" font-weight="500" letter-spacing="0.3">${escapeXml(label)}</text>
  </svg>`;
    return svgToBase64(svg);
}
/** "Voice memory" label shown above the player */
function getVoiceMemoryLabelSvg() {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="120" height="18" viewBox="0 0 120 18" fill="none">
    <text x="0" y="13" fill="${exports.COLORS.playTeal}" font-family="${FONT}" font-size="11.5" font-weight="500">Voice memory</text>
  </svg>`;
    return svgToBase64(svg);
}
/** Filled play pill + waveform, shown when the symbol DOES have a voice memory */
function getVoiceMemorySvg(durationStr) {
    ...
    <rect class="play-rect" x="0" y="0" width="76" height="26" rx="13" fill="${exports.COLORS.playTeal}"/>
    <path class="play-text" d="M16 8.5L25 13L16 17.5V8.5Z" fill="${exports.COLORS.playText}"/>
    <text class="play-text" x="48" y="17" fill="${exports.COLORS.playText}" font-family="${FONT}" font-size="11" font-weight="700" text-anchor="middle">${escapeXml(durationStr)}</text>
    <g fill="${exports.COLORS.playTeal}">
...
function getFooterPlayMemorySvg() {
    ...
    <text x="48" y="14" fill="${exports.COLORS.muted}" font-family="${FONT}" font-size="11.5" font-weight="500" text-anchor="middle">Play memory</text>
  </svg>`;
    return svgToBase64(svg);
}
/** Footer "Full Docs" button */
function getFooterFullDocsSvg() {
    ...
    <text x="40" y="14" fill="${exports.COLORS.muted}" font-family="${FONT}" font-size="11.5" font-weight="500" text-anchor="middle">Full Docs</text>
  </svg>`;
    return svgToBase64(svg);
}
/** Small outlined "Play memory" pill for the header row */
function getPlayMemoryHeaderSvg() {
    const triangle = `<path d="M15 8.5L23 13L15 17.5V8.5Z" fill="${exports.COLORS.playTeal}"/>`;
    return outlinedPill({ label: 'Play memory', color: exports.COLORS.playTeal, width: 118, leadingIcon: triangle, textX: 66 });
}
